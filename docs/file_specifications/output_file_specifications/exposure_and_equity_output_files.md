...
Regardless of user input, an exposure distribution figure is generated labeled "[batch]_[run]_PM25_Exposure_Percentiles.png". The map includes PM<sub>2.5</sub> exposure (µg/m³) by percentile among imput population groups (e.g., Total, Asian, Black, Hispanic/Latino, Native American, Pacific Islander, White, and Other). If DPM is calculated, there will be an equivalent percentile plot for DPM ("[batch]_[run]_DPM_exposure_percentiles.png").
...
* **PM 2.5(µg/m³) Concentration**: PM<sub>2.5</sub> Concentration associated with each ISRM run
* * **DPM (µg/m³) Concentration**: DPM Concentration associated with each ISRM run, if enabled
...
An exposure disparity summary table is generated labeled "[batch]_[run]_exposure_disparity.csv". This CSV file includes the group PWM (population weighted mean), absolute disparity, and relative disparity among racial groups for PM<sub>2.5</sub> and DPM (if enabled). 
